...
namespace MiNET
{
	...
	public class MinetAnvilTest
	{
		...
		[Test, Ignore("")]
		public void LoadAnvilLevelTest()
		{
			...
			Assert.AreEqual("", file.RootTag.Name);
		...
		[Test, Ignore("")]
		public void SignStrippingTests()
		{
			Regex _regex = new Regex(@"^((\{""extra"":\[)?)(\{""text"":"".*?""})(],)?(""text"":"".*?""})?$");

			//   TAG_String("Text4"): "{"extra":[{"text":"---------------"}],"text":""}"

			var text = "{\"extra\":[{\"text\":\"---------------\"}],\"text\":\"\"}";
			Assert.IsTrue(_regex.Match(text).Success);
			Assert.AreEqual("{\"text\":\"---------------\"}", _regex.Replace(text, "$3"));
